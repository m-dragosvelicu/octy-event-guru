-- init_test_db.sql
-- Idempotent schema for the integration-test Postgres+PostGIS database.
-- Mirrors the production Supabase schema expected by supabase-py via PostgREST.
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'authenticator') THEN
        CREATE ROLE authenticator LOGIN PASSWORD 'testpassword';
    END IF;
END
$$;
...
-- truncate_test_tables: cleanup helper for integration tests
...
BEGIN
    ...
    TRUNCATE ingest_runs CASCADE;
    TRUNCATE alerts CASCADE;
    TRUNCATE source_scorecards CASCADE;
    -- Do NOT truncate activities (seed data should persist)
END;
$$;
